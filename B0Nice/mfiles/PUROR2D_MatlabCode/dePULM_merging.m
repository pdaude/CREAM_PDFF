function [line_tmp_phase flag_done] = dePULM_merging(line_phase,index_ls,re_phase) 
%
    pi_2 = pi*2.0;
...
%-------------------------------------------------------
    ...
    for ii = 1:2:(length(index_ls) - 1)
        if index_ls(ii+1) - index_ls(ii) < 10;
            mm = 0;
            sum_mean = 0;
            for jj = index_ls(ii):index_ls(ii+1)
            ...
            mm = 0;
            sum_mean = 0;
            for jj = index_ls(ii):(index_ls(ii)+5)
            sum_mean = sum_mean + line_phase(jj);
            mm = mm + 1;
            end 
            mean_trail(ii) = sum_mean/mm;
            ...
            mm = 0;
            sum_mean = 0;
            for jj = (index_ls(ii+1) - 5):index_ls(ii + 1)
            sum_mean = sum_mean + line_phase(jj);
            mm = mm + 1;
            end 
