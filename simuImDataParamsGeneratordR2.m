...
spectrumName='PeanutOil';
TEtypes={'MINIMUM'};% TE IN/OPP ; IDEAL  
%TEtypes={'IDEAL','MINIMUM'};% TE IN/OPP ; IDEAL 
NTEs=[7]; %number of TE 3 5 7 9
SNRs=[50,100]; % SNRs 50-100-10
...
for TEtype=TEtypes
    ...
    for NTE=NTEs
        for SNR=SNRs
            ...
            B = exp(1i*(TE'*2*pi*B0+repmat(PHI,numel(TE),1)));
            X=[W;F];
            ...
            params.B0=YB0;
            params.R2=[r2starW,r2starF];
            params.F=reshape(F,[x,y,z])/img_r;
            params.W=reshape(W,[x,y,z])/img_r;
            params.PH=reshape(PHI,[x,y,z]);
            params.FF=XFF;
            [sse,~]=calculate_residual_doubleR2(params,algoParams,imDataParams);
